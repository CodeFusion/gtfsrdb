...
try:
    ...
    app = Flask(__name__)
    app.config['SECRET_KEY'] = 'kih388eb2ob2'
    ...
    while keep_running:
        try:
            ...
            if config['gtfsr']['trip_updates']:
                ...
                fm.ParseFromString(
                    urlopen(config['gtfsr']['trip_updates']).read()
                    )
            ...
            if config['gtfsr']['alerts']:
                fm = gtfs_realtime_pb2.FeedMessage()
                fm.ParseFromString(
                    urlopen(config['gtfsr']['alerts']).read()
                    )
            ...
            if config['gtfsr']['vehicle_positions']:
                fm = gtfs_realtime_pb2.FeedMessage()
                fm.ParseFromString(
                    urlopen(config['gtfsr']['vehicle_positions']).read()
                    )
            ...
            # Send update to any Socket.io clients informing that an update is available
            emit('update available', {'data': True})
        except:
        #else:
